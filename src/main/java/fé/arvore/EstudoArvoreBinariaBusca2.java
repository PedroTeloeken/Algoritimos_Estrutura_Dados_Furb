...
public class EstudoArvoreBinariaBusca2 <T extends Comparable<T>> extends EstudoArvoreBinariaAbstract<T>{
    /**
     * Insere um novo n� na �rvore bin�ria
     * @param info
     */
    public void inserir(T info) {
        EstudoNoArvoreBinaria<T> novo = new EstudoNoArvoreBinaria<>(info);
    ...
    private EstudoNoArvoreBinaria<T> extrairSucessor(EstudoNoArvoreBinaria<T> excluir) {
        EstudoNoArvoreBinaria<T> p = excluir.getDireita();
        EstudoNoArvoreBinaria<T> paiSucessor = excluir;
    ...
    private void removerNoDoisFilhos(DadosRemocao dados) {
        // caso 3
        EstudoNoArvoreBinaria<T> sucessor = extrairSucessor(dados.p);
        if (dados.p == getRaiz())
            setRaiz(sucessor);
        else {
            if (dados.filhoEsquerda)
